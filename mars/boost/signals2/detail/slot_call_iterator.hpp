  ...
  namespace signals2 {
    namespace detail {
      ...
      private:
        ...
        {
            std::vector<mars_boost::shared_ptr<void>> local_tracked_ptrs;
          ...
          for(;iter != end; ++iter)
          {
            local_tracked_ptrs.clear();
            (*iter)->nolock_grab_tracked_objects(lock, std::back_inserter(local_tracked_ptrs));
